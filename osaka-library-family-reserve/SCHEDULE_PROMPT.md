...
2. **母のフィードバック反映**: このスレッドでユーザー（母）から前回以降に届いた発言
   （「◯◯はもう読んだ」「◯◯面白かった」「◯◯みたいな本を」等）があれば、
   `data/mom/preferences.md` の「フィードバックの受け方」表に従って
   `data/mom/history.csv` と preferences.md の「最近の要望メモ」を更新する。

3. **母の選書**: `data/mom/preferences.md` を読み、そこに書かれた選書ポリシー
   （直木賞・芥川賞・本屋大賞・山本周五郎賞・日本ミステリー文学大賞の受賞作・候補作と
   その作家の他作品がベース）に従って今週の4冊を Web 検索で選書する。
   history.csv・recommended.json・queue.json 残にある本は除外し、評価と要望メモを反映して
   `data/mom/queue.json` に書く:
   `{ "items": [ { "title": "書名", "author": "著者", "reason": "選んだ理由" } ] }`

4. **シリーズファイルの整備**: 次の2つを行う。
...
5. **計画確認**: `./run.sh --all --plan-only` を実行（数秒で終わる・サイトアクセスなし）し、
   今週の予約予定リストに異常がないか確認する（重複や明らかな選書ミス）。異常があれば報告のみ。

6. **本番実行（刻み実行）**: `./run.sh --tick` を、**出力の最後に `TICK:DONE` が出るまで繰り返し実行**する。
   - 1回の実行は45秒以内に終わる小さなステップで、週の合計はおよそ30〜50回
   - 途中でコマンドが強制終了・タイムアウトしても、もう一度実行すれば続きから自動再開する
   - エラーが出た回があっても、そのまま再実行してよい（自動で再ログインして復旧する）
   - 進捗は毎回1行表示される（例: `TICK:CONTINUE chojo-done「注文の多い料理店」`）

7. **記録と報告**:
   - 変更されたファイル（state/, reports/, data/）をコミットする。git がロックエラーを出す場合は
     先に `rm -f .git/index.lock` を実行してよい。コミットメッセージ例: `weekly reserve 2026-07-05`。
     **.env は絶対にコミットしない。**
   - プッシュを試み、GitHub認証が無い等でプッシュできなければ**スキップして構わない**
     （このフォルダは永続なので状態はローカルに保持される。プッシュはバックアップ用途）。
   - `reports/YYYY-MM-DD.md` の内容を要約してユーザーに報告する:
     アカウント別の予約成功／失敗／スキップ理由、シリーズ展開待ちの有無、次回の開始位置。
     母の4冊はタイトル・作家・選んだ理由（どの賞・どの評価に基づくか）も添える。
   - 報告の最後に必ず添える:

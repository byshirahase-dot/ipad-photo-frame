...
- **予約枠**: ログイン後に予約中冊数を取得し、上限（accounts.json: reserveLimit=15）を超える分は
  予約せずレポートに記載
- **期限切れ予約の自動復帰（2026-07-03追加）**: 毎回ログイン後に予約状況一覧を確認し、
  予約状態が「期限切れ」「無効」の本（取り置き期限切れ・延滞ペナルティ等で借りられなかった本）は
  台帳エントリを expired にして再予約対象へ戻す（公文=キュー先頭に割込み、母=queue.json先頭へ）。
  ※手動の「取消」は対象外。サイトの実際のラベルが違った場合は src/index.js の
  `/期限切れ|無効/` を実ラベルに合わせて調整する（予約状態の値は logs/ の rsv-list スクショで確認可能）。
  既知の制約: 母は queue.json が空の週はログインしないため、期限切れ検出も次の選書週まで持ち越し

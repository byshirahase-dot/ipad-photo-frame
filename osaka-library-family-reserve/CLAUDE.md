...
1. **長女は通常どおり実行してよい**（アカウントは正常と判明）。進度 E-5＝きまぐれロボットから。
   注文の多い料理店は queue 除外済みなので「既に貸出中」の巻き添えは起きない。もし成立0が続く/
   「所蔵なし」が多発する場合はサイトのスロットリングを疑い、時間を空けて1回だけ再実行する。
2. `git pull` で本ブランチを最新化（各 progress/reserved/queue を引き継ぐ）。認証は環境変数から。
3. 通常の週次運用（SCHEDULE_PROMPT.md 手順0〜7）を4アカウント実行（--limit は付けない）。
   母の queue は上記4冊が入ったまま。長男・次男は取消復帰分が queue 先頭。

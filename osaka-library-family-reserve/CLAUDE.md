...
- **長女（chojo）: 今週0冊＝サイト側スロットリング（アカウントは正常）**。
...
1. **長女（chojo）は 07-27 にスロットリングで0冊。アカウントは正常**。進度 E-6＝リトルバンパイア1/2 →
   竜退治 → ぼくは、いつでもぼくだった。から通常実行される（状態は 07-27 に実行前へ完全復元済み）。
   もし再び成立0や「所蔵なし」多発が続く場合はスロットリングを疑い、**時間を空けて1回だけ**再実行する
   （そのとき _expanded.json も git checkout で巻き戻すこと＝中断時に即 persist される既知の落とし穴）。

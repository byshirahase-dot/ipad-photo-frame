...
0. **⛔ 長女(chojo)は 2026-08-10 の実行をスキップ（ユーザー指示 2026-07-30）**。
   （2026-08-03 のスキップは実施済み＝セッション7。残るは **08-10 の1回のみ**。）
   この回は **`./run.sh --account=chojo` を実行しない**（母・長男・次男の3アカウントのみ実行）。
   **2026-08-17 の回から chojo を通常どおり再開**する。再開時は state はそのまま引き継がれ、
   queue 先頭の「にんじん」→リトルバンパイア… の順で予約される。08-10 のスキップを終え 08-17 で再開したらこの項目を削除。
   ※長女リクエスト: 公文G-4『にんじん』(ルナール/新潮社)を先に読みたいとのことで、2026-07-30 に
   `state/chojo/queue.json` 先頭へ前倒し投入済み（進度E-6より先に予約される）。--plan-only 確認済み。

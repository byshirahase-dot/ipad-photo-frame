...
- **⚠️ 長女（chojo）: 今週0冊・アカウント異常**。ログイン後の予約一覧が「予約中0件・[取消]32件」。
  新規予約は「既に貸出中の書誌です。予約できません。」で軒並み拒否。先週まで有効だった予約も一覧から
  消え、過去分が [取消] に。**延滞ペナルティ or 取り置き期限切れによる予約停止・一括取消と推定**。
  32件の取消の大半は本システム外の本で安全ガードにより台帳は無変更。**コード修正はせず報告のみ**。
  進度は E-5 据え置き、queue（注文の多い料理店の版違い再予約）も残置。再実行は1回まで実施済み。
  read-only `verify-reservations.mjs chojo` で実測（要 OML_CHROMIUM_PATH を export してから実行）。
  リトルバンパイア 1 の「検索ヒットなし」はアカウント異常に伴う一時的なものと判断し、台帳の
  failed 記録は取り消した（来週再検索させる。先週 OPAC で所蔵確認済みの巻を恒久的に落とさないため）。
...
1. **長女アカウントの復旧確認が最優先**。延滞・ペナルティが解消していれば通常どおり4冊再試行される。
   まだ [取消]/「既に貸出中」が続く場合は再びユーザーへ報告（コード修正はしない）。

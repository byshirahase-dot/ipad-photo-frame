...
1. **長女は通常どおり実行してよい**（アカウントは正常と判明）。進度 E-6＝リトルバンパイアから
   （表記問題は修正済みで自動展開される）。もし成立0が続く/「所蔵なし」が多発する場合はサイトの
   スロットリングを疑い、時間を空けて1回だけ再実行する。注文の多い料理店は queue 除外済み。
...
3. 通常の週次運用（SCHEDULE_PROMPT.md 手順0〜7）を4アカウント実行（--limit は付けない）。
   **取消復帰の誤判定は 2026-07-20 に修正済み**（借用中の本は borrowed 終了扱いで再キューされない）。
   母 queue は wishlist 最優先の4冊。長男 queue は 11ぴきのねこどろんこ、次男 queue は14ひきシリーズ。

...
0. **⛔ 長女(chojo)は 2026-08-03 と 2026-08-10 の実行をスキップ（ユーザー指示 2026-07-30）**。
   この2回は **`./run.sh --account=chojo` を実行しない**（母・長男・次男の3アカウントのみ実行）。
   **2026-08-17 の回から chojo を通常どおり再開**する。再開時は state はそのまま引き継がれ、
   queue 先頭の「にんじん」→リトルバンパイア… の順で予約される。完了したらこの項目を削除。

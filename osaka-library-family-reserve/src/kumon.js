...
/**
 * カート投入フェーズの処理順を最適化する（2026-08-09 追加 / 2026-08-17 全面見直し）。
 *
 ...
 * 巻き添えで全滅する事故があった（jinan 2026-08-10・2026-08-17 いずれも「まよなかのだいどころ」）。
 *
 * ★2026-08-17 の真因: 版スキップの主因は「出版社（publisher）指定のある本」＝リスト指定の版が
 * 見つからないと candidates 0 で見送りになる。ところが問題の「まよなかのだいどころ」は公文リスト本
 * （from="list"・publisher=冨山房）で、planWeek では公文本体がキュー本の後に積まれるため picks の
 * **末尾**に来る。旧実装は from が "requeue" で始まる本だけを「後ろ」に回していたが、
 * 取消復帰本の from は "requeue:..." のほか "再予約リクエスト(...)" もあり、さらに末尾の版スキップ本は
 * from="list" だったため、**後ろに回す群が空＝並べ替えが起きず**、版スキップ本が末尾のまま残っていた。
 *
 * 対策（判定軸を from から publisher へ）: 版ガードで見送りになりやすい本＝**publisher 指定のある本**を
 * 先に、版ガードが無く確実にカートへ入る本＝**publisher 指定の無い本**（取消復帰など、以前実際に
 * 予約できた版が存在する本は publisher を持たない）を後に処理する。最後の操作が addToCart（カート画面に
 * 留まる）になり、確定フェーズがカートを見失わない。安定ソートで各群内の順序は保つ。
 *
 * これは予防策。publisher 指定の無い本が1冊も無い週（全冊が版指定本）では末尾が版スキップになり得るため、
 * 確定側（opac.reserveCartContents / #openCart）が「投入済みの本の書誌詳細URLを開き直してカートを
